...
#{{{ Initialization of Properties
        ...
        if self.batch_fn is not None:
            self._inputs_type = 'fn'
        else:
            self._inputs_type = 'numpy'

        if self._inputs_type == 'numpy':
            self._inputs = inputs
            ...
            if masks is not None:
                self.has_masks = True
                self._masks = masks
        elif self._inputs_type == 'fn':
            print 'training with batch function'
            try:
                batch = self.batch_fn(no_step=True)
            except TypeError:
                batch = self.batch_fn()
            self.numloads = 1
            if numloads is not None:
                self.numloads = numloads
            self.loadsize = batch['inputs'].shape[0]
            if 'targets' in batch:
                self.is_supervised = True
            if 'masks' in batch:
                self.has_masks = True
...
#}}}
    ...
    def _trainsubstep(self, batchidx):
        ...
        stepcost, avg_gradnorm, any_isnan = self._updateincs()
        # catch NaN, before updating params
        try:
            if np.isnan(stepcost) or any_isnan:
                raise ValueError('Cost or gradient is nan!')
    ...
    def upload_data(self, loadidx):
        ...
        if self._inputs_type == 'numpy':
            ...
            load = self.batch_fn()
            self._inputs_theano.set_value(
                self.preprocess_fn(load['inputs']))
            if self.is_supervised:
                self._outputs_theano.set_value(
                    load['targets'])
            if self.has_masks:
        ...
        for load_index in range(self.numloads):
            ...
            if self.verbose and not self.monitor_by_epoch_not_load:
                ...
                if hasattr(self._model, 'monitor'):
                    self._model.monitor()
            if self.monitor_update_weight_norm_ratio and not self.monitor_by_epoch_not_load:
